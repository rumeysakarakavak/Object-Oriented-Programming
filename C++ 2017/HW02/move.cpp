...
// TODO : file ismi
char getMove(int size, string& fileName) {
    char character;
    string command;
    ...
    if (command != "SAVE" && command != "LOAD"){
        ...
        while(!((character>='a' && character<='a' + size -1) || (character>='A' && character<='A'+ size -1 )
                || character == '#' || character == '$')){
            cerr << "Enter A-" << static_cast<char> ('A' + size-1) << std :: endl;
...
int makeMove(int size, char **board, char playerCharacter, string& fileName, char players){

    int win = 0;
    char movement = getMove(size, fileName);
    // if command is SAVE
    if(movement == '$') {
        cout << "SAVED" << endl;
        // open a file and write on them.
        saveFile(fileName, board, size, players);
        // go on play
...
int player1(int size, char **board, string& fileName, char players) {
    std :: cout << "Turn of player X";
    std::cout << std::endl;
    return makeMove(size, board, 'X', fileName, players);;
}
// Turn of player 2 -> calls makeMove
int player2(int size, char **board, string& fileName, char players) {
    std :: cout << "Turn of player O";
    std::cout << std::endl;
    return makeMove(size, board, 'O', fileName, players);;
}
// Who will play the game? -> user2user or user2computer
void playTogether(int size, char **board, char players, string& fileName) {
    int win = 0;
    if(players == 'P' || players == 'p'){
        while(win != 1 && checkAllBoard(board, size) != 1){
            win = player1(size, board, fileName, players);
            if(win != 1){
                win =player2(size, board, fileName, players);
            }
        }
    }
    else{
    	while(win != 1 && checkAllBoard(board, size) != 1){
            win = computer(size, board, 'X', fileName, players);
    	}
...
// Play with computer
int computer(int size, char **board, char playerCharacter, string& fileName, char players) {
